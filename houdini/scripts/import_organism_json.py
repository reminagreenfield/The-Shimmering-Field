...
def main():
    ...
    for name in ("id", "age", "generation", "parent_id", "merger_count",
                 ...
                 "is_mature", "role_id"):
        ensure_point_attrib(geo, hou.attribType.Point, name, 0)
    ...
    for name in ("energy", "genomic_stress", "viral_load", "lysogenic_strength",
        ...
        ensure_point_attrib(geo, hou.attribType.Point, name, 0.0)

    # Strings
    for name in ("modules", "trophic_role"):
        ensure_point_attrib(geo, hou.attribType.Point, name, "")

    # Per-module presence: i@has_PHOTO, i@has_CHEMO, ...
    for mod in MODULE_NAMES:
        ensure_point_attrib(geo, hou.attribType.Point, "has_" + mod, 0)

    # Per-module active: i@active_PHOTO, ...
    for mod in MODULE_NAMES:
        ensure_point_attrib(geo, hou.attribType.Point, "active_" + mod, 0)

    # Per-module usage: f@usage_PHOTO, ...
    for mod in MODULE_NAMES:
        ensure_point_attrib(geo, hou.attribType.Point, "usage_" + mod, 0.0)
