"""
The Shimmering Field — Phase 3 Step 3: Consumptive Module
============================================================
Adds CONSUME module: predation and scavenging with four trophic roles.

Mechanics:
  - CONSUME weights: [prey_selectivity, handling_efficiency, decomp_preference, aggression]
  - Asymmetric metabolic interference: omnivores hunt at 30%, produce at 85%
  - Gaussian scent layer: detritivores smell death zones from ~15 cells
  - Per-cell kill cap + predator satiation dampens Lotka-Volterra oscillations
  - Density-dependent reproduction prevents extreme population spikes

Trophic roles (emergent):
  - Producers:    PHOTO and/or CHEMO, no CONSUME
  - Carnivores:   CONSUME only, low decomp_preference (hunters)
  - Detritivores: CONSUME only, high decomp_preference (scavengers)
  - Omnivores:    producer + CONSUME (penalized at both)

Built on Phase 3 Step 2 (CHEMO module).
"""
...
MODULE_WEIGHT_SIZES = np.array([
    ...
    0, 0, 0, 0, 0,  # FORAGE, DEFENSE, DETOX, TOXPROD, VRESIST (future)
], dtype=np.int32)
...
TOTAL_MODULE_WEIGHTS = _off  # 20

N_STANDALONE_PARAMS = 4
STANDALONE_OFFSET = TOTAL_MODULE_WEIGHTS
TOTAL_WEIGHT_PARAMS = TOTAL_MODULE_WEIGHTS + N_STANDALONE_PARAMS  # 24
...
MODULE_MAINTENANCE = np.array([0.20, 0.25, 0.12, 0.08, 0.15, 0.20, 0.25, 0.03, 0.15])
MODULE_EXPRESSION  = np.array([0.10, 0.12, 0.08, 0.06, 0.08, 0.08, 0.12, 0.02, 0.08])
BASE_MAINTENANCE = 0.05
# Total cost examples:
#   PHOTO+MOVE+TOXPROD             = 0.54
#   + CHEMO                        = 0.91
#   + CONSUME (all energy modules) = 1.11

GAINABLE_MODULES = [M_CHEMO, M_CONSUME, M_MOVE]
...
class World:
    ...
    def _init_organisms(self):
        ...
        # Weight offsets: CONSUME starts at offset 8, decomp_pref=w[10], aggression=w[11]
        self.weights[carn_idx, 11] = self.rng.uniform(1.0, 2.5, n_carn)   # high aggression
        self.weights[carn_idx, 10] = self.rng.uniform(-3.0, -1.5, n_carn)  # low decomp_pref
        ...
        self.weights[detr_idx, 10] = self.rng.uniform(1.5, 3.0, n_detr)   # high decomp_pref
        self.weights[detr_idx, 11] = self.rng.uniform(-2.0, -0.5, n_detr)  # low aggression
    ...
    def _acquire_energy(self, readings):
        ...
        if has_photo.any():
            ...
            total_gain += (eff * tp * lm * st * sh * prod_mult) * has_photo
        ...
        if has_chemo.any():
            ...
            total_gain += (eff * toxic_factor * sh * prod_mult) * has_chemo
        ...
        if has_consume.any():
            ...
            scavenge_gain = eff * handling * available
            total_gain += scavenge_gain * has_consume

            # Deplete local decomp
    ...
    def _predation(self):
        """Vectorized predation with per-cell kill cap and satiation."""
        c = self.cfg
        n = self.pop
        if n < 2:
            return 0
        ...
        # Build spatial index using numpy
        cell_key = self.rows * N + self.cols
        sort_idx = np.argsort(cell_key)
        sorted_keys = cell_key[sort_idx]
        # Find boundaries for each cell
        unique_cells, cell_starts, cell_counts = np.unique(
            sorted_keys, return_index=True, return_counts=True)
        cell_lookup = {}
        for i, (uc, cs, cc) in enumerate(zip(unique_cells, cell_starts, cell_counts)):
            cell_lookup[int(uc)] = sort_idx[cs:cs+cc]
        ...
        for pi in predator_idx:
            ...
            # Success probability
            hunt_skill = aggression[pi] * (1.0 - 0.5 * decomp_pref[pi])
            mobility_bonus = 0.08 if has_move[pi] else 0.0
            spec_bonus = 0.15 if is_specialist[pi] else 0.0
            target_difficulty = np.clip(self.energy[ti] / c.energy_max, 0.1, 1.0)
            prob = (c.predation_base_success + hunt_skill * 0.3
                    + mobility_bonus + spec_bonus - target_difficulty * 0.08)
            if is_omnivore[pi]:
                prob *= c.producer_consume_penalty
            prob = np.clip(prob, 0.01, 0.60)

            if self.rng.random() < prob:
                frac = c.predation_energy_fraction
                if is_specialist[pi]:
                    frac *= (1.0 + c.consumer_specialist_bonus)
                gained = max(0.0, self.energy[ti]) * frac
                ...
                # Satiation: fed predators likely skip further hunting this step
                if self.rng.random() < c.predator_satiation:
                    satiated[pi] = True
    ...
    def _apply_toxic_damage(self, readings):
        ...
        m = effective > c.toxic_threshold_medium
        if m.any():
            dmg[m] += (effective[m] - c.toxic_threshold_medium) * c.toxic_damage_medium
        h = effective > c.toxic_threshold_high
        if h.any():
            dmg[h] += (effective[h] - c.toxic_threshold_high) * c.toxic_damage_high
        self.energy -= dmg
    ...
    def _decide_movement(self, readings):
        ...
        gy = light_w * lg_y - net_toxic * tg_y + consume_dens_w * dg_y + consume_scent_w * sg_y
        gx = light_w * lg_x - net_toxic * tg_x + consume_dens_w * dg_x + consume_scent_w * sg_x
        sc[:, 1] = gy;  sc[:, 2] = -gy
        sc[:, 3] = gx;  sc[:, 4] = -gx
    ...
    def update(self):
        ...
        self.energy = np.minimum(self.energy + self._acquire_energy(readings), self.cfg.energy_max)
        self._apply_toxic_damage(readings)
        self.energy -= self._compute_module_costs()
...
def run_simulation(cfg=None):
    ...
    print(f"The Shimmering Field — Phase 3 Step 3: Consumptive Module")
    ...
    for t in range(cfg.total_timesteps):
        ...
        if world.timestep % cfg.snapshot_interval == 0:
            ...
            print(
                ...
                f"MV={mc['MOVE']:4d}  |  kill={s['kills']:3d}  |  mod={s['avg_modules']:.2f}  |  {el:.1f}s"
            )
