"""
The Shimmering Field — Phase 2 Step 2: Viral System
=====================================================
Adds lytic and lysogenic viral dynamics on top of Step 1.

New mechanics:
  - Free viral particles on grid: diffuse, decay, released by lytic deaths
  - Lytic infection: particles infect organisms → energy drain → death → particle burst
  - Lysogenic integration: some infections integrate silently into host genome
  - Toxic-triggered activation: lysogenic material activates under stress → becomes lytic
  - Lysogenic material passed vertically to offspring
  - Two new evolvable genome params: viral_resistance, lysogenic_suppression
"""
...
class World:
    def __init__(self, cfg=None):
        ...
        # Genome fragment pool (horizontal transfer from decomp)
...
def run_simulation(cfg=None):
    cfg = cfg or Config()
    world = World(cfg)
    print(f"The Shimmering Field — Phase 2 Step 2: Viral System")
    print(f"Grid: {cfg.grid_size}x{cfg.grid_size}  |  Pop: {cfg.initial_population}  |  Genome: {cfg.genome_size}")
    print(f"Viral: inf_rate={cfg.viral_infection_rate}  lyso_prob={cfg.lysogenic_probability}  activation_tox={cfg.lysogenic_activation_toxic}")
    print(f"{'─' * 105}")
    start = time.time()
    for t in range(cfg.total_timesteps):
        ...
        if world.timestep % cfg.snapshot_interval == 0:
            ...
            print(f"  t={s['t']:5d}  |  pop={s['pop']:5d}  |  e={s['avg_energy']:5.1f}  |  "
                  ...
                  f"vir={s['viral_mean']:.3f}/{s['viral_max']:.1f}  |  "
                  f"lytic={s['n_lytic']:4d} lyso={s['n_lysogenic']:4d}  |  "
                  f"Ldeath={s['total_lytic_deaths']:5d} Lint={s['total_lyso_integrations']:5d} "
                  f"Lact={s['total_lyso_activations']:5d}  |  {el:.1f}s")
    ...
    print(f"Done in {el:.1f}s  |  Pop: {world.pop}  |  Lytic deaths: {world.total_lytic_deaths}  |  "
          f"Lysogenic integrations: {world.total_lysogenic_integrations}  |  Activations: {world.total_lysogenic_activations}")
