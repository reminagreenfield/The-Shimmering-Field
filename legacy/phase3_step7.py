"""
The Shimmering Field — Phase 3 Step 7: Reproductive Manipulation
================================================================
Lysogenic viral material silently manipulates host reproduction (Wolbachia-style).

Not a new module — operates through the existing lysogenic genome system.
When a parent has lysogenic_strength above threshold, viral material:
  1. Trait bias:     pushes offspring weights toward the lysogenic genome (homogenization)
  2. Receptivity:    boosts offspring transfer receptivity (spreads manipulation further)
  3. Viability:      penalizes offspring that diverge from the lysogenic template
  4. Self-limiting:  manipulation weakens at high population penetration (frequency-dependent)

Diversity collapse trajectory:
  - Phase A: silent spread (lysogenic material integrates via normal viral dynamics)
  - Phase B: apparent flourishing (manipulated offspring are fit because they match template)
  - Phase C: brittle exposure (population homogenized, novel stresses find no variation)

Key interactions:
  - VRESIST suppresses lysogenic integration → immune organisms resist manipulation
  - High toxicity triggers lysogenic activation → manipulation intensifies under stress
  - HGT spreads lysogenic material laterally → manipulation propagates spatially
  - Nutrient cycling + mediator networks mask homogenization (population looks healthy)

Built on Phase 3 Step 6 (MEDIATE + Nutrient Cycling + Herbivore gradient).
"""
...
def run_simulation(cfg=None):
    ...
    print(f"The Shimmering Field — Phase 3 Step 7: Reproductive Manipulation")
