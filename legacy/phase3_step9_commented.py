"""
The Shimmering Field — Phase 3 Step 9: Endosymbiosis (Phase 3 Capstone)
========================================================================
Two organisms merge into a composite entity — the culmination of all
relational mechanics built in Steps 5-8.

PHASE 3 COMPLETE ARCHITECTURE
==============================
This is the capstone of Phase 3. All 11 modules, full two-layer genome,
differentiated ecological roles, and every parasitic/mutualistic system active.

Module inventory (11 modules, up to 48 evolvable weights per organism):
  ┌──────────────────────────────────────────────────────────────────┐
  │  ENERGY PRODUCTION           │  INTERACTION                     │
  │  ├─ PHOTO  (4 wts)           │  ├─ CONSUME  (4 wts)             │
  │  ├─ CHEMO  (4 wts)           │  ├─ DEFENSE  (4 wts)             │
  │  └─ FORAGE (4 wts)           │  └─ SOCIAL   (4 wts)             │
  │                              │                                  │
  │  METABOLISM                  │  INFRASTRUCTURE                  │
  │  ├─ DETOX   (4 wts)          │  ├─ MOVE     (8 wts)             │
  │  └─ TOXPROD (0 wts)          │  └─ MEDIATE  (4 wts)             │
  │                              │                                  │
  │  IMMUNE                      │  STANDALONE  (4 wts)             │
  │  └─ VRESIST (4 wts)          │  └─ transfer, viral, lyso params │
  └──────────────────────────────────────────────────────────────────┘

Ecological roles (emergent from module combinations):
  - Producer:    PHOTO/CHEMO, no CONSUME        → sessile "plant" or mobile "plankton"
  - Herbivore:   CONSUME, low prey_selectivity   → grazes producers
  - Carnivore:   CONSUME, high prey_selectivity  → hunts consumers
  - Detritivore: CONSUME, high decomp_preference → scavenges dead matter
  - Omnivore:    PHOTO/CHEMO + CONSUME           → penalized generalist

Parasitic/mutualistic systems layered across Steps 5-9:
  Step 5: VRESIST immune system → infection resistance, immune memory
  Step 6: MEDIATE pollinator networks + nutrient cycling from module interactions
  Step 7: Reproductive manipulation (Wolbachia-style lysogenic genome bias)
  Step 8: Behavioral hijacking (Toxoplasma/cordyceps movement override)
  Step 9: Endosymbiosis (two organisms merge into composite) ← THIS FILE

Endosymbiosis pathway:
  Prerequisites (ALL must be met):
    1. SOCIAL module on both organisms (relationship_score ≥ 1.5)
    2. Metabolic complementarity (≥2 different modules between partners)
    3. Co-location in same cell
    4. Medium environmental stress (toxic 0.05–1.0 sweet spot)
    5. Both have energy ≥ 50
    6. Neither has active lytic infection

  Merger event:
    - Creates one new composite organism
    - Module inventory = union of both parents' modules (capped at 8)
    - Weights = 50/50 blended from both parents
    - Energy = (sum of both) × 1.2 bonus (capped at 150)
    - merger_count incremented (lineage tracking for art pipeline)
    - Both original organisms removed

Key interactions:
  - SOCIAL relationship_score is the gate — requires sustained compatible proximity
  - Metabolic complementarity ensures merger creates something genuinely novel
  - Stress window: too peaceful = no pressure, too harsh = can't survive merging
  - VRESIST organisms 50% less likely to merge (immune system treats partner as foreign)
  - Composite organisms have more modules → higher maintenance + movement penalty
  - Movement complexity penalty (0.05 per module beyond 2) pressures composites to
    become sessile or shed modules, creating evolutionary trade-offs

Also new in this step vs Step 8:
  - Sessile producers: 40% of initial pop starts WITHOUT MOVE, get 50% photo bonus
  - Movement complexity penalty: more modules = heavier = MOVE costs more
  - merger_count field tracks endosymbiotic lineage depth
  - Sessile crowding penalty removed (sessile producers compete via canopy/roots)

Built on Phase 3 Step 8 (Behavioral Hijacking).
"""
...
# Modules can be gained through mutation (0.5%) or lost (0.3%) during reproduction,
# and gained through horizontal gene transfer from geological strata.

M_PHOTO   = 0   # Photosynthesis: light → energy. Primary producer pathway.
M_CHEMO   = 1   # Chemosynthesis: toxin → energy. Thrives in toxic zones.
M_CONSUME = 2   # Consumption: eat other organisms (predation) or decomp (scavenging).
M_MOVE    = 3   # Locomotion: move 1 cell/step in 4 directions. Costs scale with module count.
M_FORAGE  = 4   # Enhanced gathering: extraction bonus + cooperative energy + storage.
M_DEFENSE = 5   # Anti-predator: shell (block), camouflage (hide), counter-attack (damage).
M_DETOX   = 6   # Toxin metabolism: clean environment + gain energy from toxins.
M_TOXPROD = 7   # Toxic waste: always present, produces environmental toxicity. No weights.
M_VRESIST = 8   # Viral resistance: immune system with memory. Resists hijacking.
M_SOCIAL  = 9   # Social signaling: identity broadcast + relationship building (endo gate).
M_MEDIATE = 10  # Pollination/dispersal: lowers reproduction threshold for nearby organisms.
...
TOTAL_MODULE_WEIGHTS = _off  # 32 (PHOTO[0:4] CHEMO[4:8] CONSUME[8:12] MOVE[12:20] FORAGE[20:24] DEFENSE[24:28] DETOX[28:32])
# Note: TOXPROD contributes 0 weights. VRESIST/SOCIAL/MEDIATE not counted here because
# the original code comment says "# 32" — but dynamically this computes to 44 with those
# three modules (32 + 4 + 4 + 4 = 44). The actual value is whatever _off ends up as.

N_STANDALONE_PARAMS = 4   # 4 parameters not tied to any module
STANDALONE_OFFSET = TOTAL_MODULE_WEIGHTS  # Index where standalone params start
TOTAL_WEIGHT_PARAMS = TOTAL_MODULE_WEIGHTS + N_STANDALONE_PARAMS  # Total genome length

# Standalone parameter indices (offset from STANDALONE_OFFSET):
SP_TRANSFER_RECEPTIVITY = 0  # How readily organism absorbs DNA from strata
SP_TRANSFER_SELECTIVITY = 1  # How picky about DNA quality (higher = more selective)
SP_VIRAL_RESISTANCE = 2      # Baseline (non-VRESIST) viral resistance
SP_LYSO_SUPPRESSION = 3      # Baseline lysogenic suppression (prevents dormant virus waking)

# Module costs: energy drain per step for having (maintenance) and using (expression).
# An organism pays maintenance for every module it has, plus expression for every active one.
...
BASE_MAINTENANCE = 0.05  # Every organism pays this just to exist
# Cost examples:
...
# Configuration
# ─────────────────────────────────────────────────────
# All simulation parameters in one place. Changing these tunes the ecology.
# Parameters are organized by system. Step 9 adds endosymbiosis params.
...
class Config:
    ...
    # Behavioral hijacking (Step 8: Toxoplasma/cordyceps-style)
    # Lytic virus doesn't just damage — it takes over host behavior.
    # Hijacked organisms seek dense crowds (to spread virus) and lose defensive ability.
    # Creates a parasitic manipulation layer distinct from reproductive manipulation.
    hijack_load_min = 0.05             # Min viral_load for any hijack effects
    hijack_load_heavy = 0.5            # Load threshold for full-intensity hijack
    hijack_defense_suppress = 0.6      # Max 60% defense suppression (can't fight back)
    hijack_vresist_suppress = 0.4      # Max 40% immune suppression (can't clear virus)
    hijack_energy_suppress = 0.7       # Max 70% energy acquisition blocked (starving host)
    hijack_density_seek = 3.0          # Movement bias weight toward crowded areas
    hijack_stress_amplifier = 1.5      # Toxic stress makes hijack 50% more intense

    # Endosymbiosis (Step 9: THIS STEP — the Phase 3 capstone)
    # Two compatible organisms in the same cell merge into one composite entity.
    # Requires sustained social relationship, metabolic complementarity,
    # moderate environmental stress, and sufficient energy. Rare but transformative.
    endo_check_interval = 10             # Check every 10 steps (rare event)
    endo_relationship_threshold = 1.5    # Min relationship_score for BOTH partners
    endo_energy_threshold = 50.0         # Both need ≥50 energy to merge
    endo_toxic_min = 0.05                # Min stress (no pressure → no merger)
    endo_toxic_max = 1.0                 # Max stress (too harsh → can't survive merging)
    endo_complementarity_min = 2         # Must differ in ≥2 modules (ensures novelty)
    endo_probability = 0.15              # Base 15% merger probability when all conditions met
    endo_vresist_penalty = 0.5           # 50% probability reduction if either has VRESIST
    endo_weight_blend = 0.5              # Equal 50/50 blend of both parents' weights
    endo_energy_bonus = 1.2              # 20% energy bonus for merged organism (synergy)
    endo_max_modules = 8                 # Cap on modules in composite (more = fatal costs)
    ...
    output_dir = "output_p3s9"
    random_seed = 42


# ─────────────────────────────────────────────────────
# World — simulation state: environment grids + organism arrays
# ─────────────────────────────────────────────────────
# All organisms stored as parallel numpy arrays (SoA layout) for performance.
# Environment is layered grids: light, toxic, nutrients, decomp, viral, social, mediator.
...
class World:
    def __init__(self, cfg=None):
        ...
        self.total_mergers = 0         # NEW Step 9: total endosymbiotic merger events

    def _init_organisms(self):
        """Seed initial population: producers (60% mobile, 40% sessile), carnivores, detritivores.
        
        NEW IN STEP 9: 40% of producers start WITHOUT MOVE (sessile).
        Sessile producers get a 50% photosynthesis bonus (better light orientation,
        deeper roots) but can never escape predators or relocate to better light.
        Also: merger_count field tracks endosymbiotic lineage depth.
        """
        c = self.cfg
        N = c.grid_size
        ...
        # NEW Step 9: Sessile producers — 40% start without MOVE.
        # These are "rooted plants" that photosynthesize 50% better (sessile_bonus in _acquire_energy)
        # but can never escape predators or relocate to better light.
        # Creates a meaningful strategic split: sessile (high production) vs mobile (flexibility).
        ...
        # Endosymbiosis: merger history (NEW in Step 9)
        # 0 = never merged, 1 = first-generation composite, 2 = composite of composites, etc.
        # Inherited by offspring. Tracks endosymbiotic lineage depth for art pipeline.
    ...
    def _compute_total_costs(self):
        ...
        NEW IN STEP 9: Movement complexity penalty.
        Each module beyond 2 adds 0.05 to MOVE cost per step.
        A 3-module mobile organism pays 0.05 extra, 5-module pays 0.15 extra, etc.
        This makes heavy composites (from endosymbiosis) expensive to move,
        creating evolutionary pressure for composites to either become sessile
        or shed unnecessary modules.
        """
        c = self.cfg
        ...
        # NEW Step 9: Movement complexity penalty — more modules = heavier = MOVE costs more.
        # This is the key cost pressure on endosymbiotic composites:
        # A 6-module composite pays 0.20 extra per step to move, a 8-module pays 0.30.
        # Combined with high base maintenance, this makes rich composites very expensive
        # to keep mobile, creating evolutionary pressure toward sessility or module loss.
    ...
    # All per-organism array field names (used by _filter and _append).
    # These must be kept in sync — every organism property is a parallel array.
    # Step 9 adds: merger_count (endosymbiotic lineage depth).
    _ORG_FIELDS = [
        ...
        "immune_experience", "relationship_score", "merger_count",
    ]
    ...
    def _sense_local(self):
        """Each organism reads 22 sensor channels from its local environment.
        
        Channels 0-14: Physical environment (light, toxic, nutrients, density, decomp + gradients)
        Channels 15-20: Social field signals and gradients (Step 5+)
        Channel 21: Mediator field density (Step 6+)
        
        Gradients = (cell_above - cell_below) for Y, (cell_right - cell_left) for X.
        Positive gradient = "more in that direction" — organisms use these for navigation.
        """
    ...
    def _acquire_energy(self, readings):
        ...
        if has_photo.any():
            ...
            # NEW Step 9: Sessile bonus — rooted producers without MOVE photosynthesize 50% better.
            # Better light orientation, deeper nutrient roots, no energy wasted on locomotion.
            # Creates meaningful strategic split: sessile (high output) vs mobile (flexibility).
    ...
    def _compute_hijack_intensity(self):
        """Compute per-organism hijack intensity from viral load (Step 8).
        
        Returns array of 0-1 values stored on self.hijack_intensity.
        Called once per step — other systems read this to modify behavior.
        
        Intensity ramps from 0 at viral_load=0.05 to 1.0 at viral_load=0.5.
        Toxic stress amplifies (stressed hosts = easier to control).
        VRESIST partially resists via breadth + immune memory.
        
        Effects applied elsewhere:
          _predation():         defense suppression (can't fight predators)
          _acquire_energy():    energy acquisition suppressed up to 70%
          _decide_movement():   movement overridden toward dense crowds
        """
        c = self.cfg
        n = self.pop
    ...
    def _attempt_endosymbiosis(self):
        ...
        Uses spatial grouping to find pairs in the same cell.
        
        This is the Phase 3 capstone mechanic. Two organisms merge into one composite
        entity — the culmination of social, metabolic, and environmental pressure systems.
        
        Pipeline:
          1. Filter to eligible organisms (SOCIAL, high relationship, energy, no virus)
          2. Filter by toxic stress window (sweet spot: not too safe, not too harsh)
          3. Group by cell, find co-located pairs
          4. Check metabolic complementarity (≥2 module differences)
          5. Roll probability (base 15%, boosted by relationship, penalized by VRESIST)
          6. Execute merger: union modules (cap 8), blend weights, sum energy × 1.2
          7. Remove both partners, increment merger_count
          
        The module cap (8) with priority-based dropping creates interesting choices:
        MEDIATE (lowest priority) gets dropped first, then VRESIST, then DETOX.
        Energy modules (PHOTO, CHEMO, CONSUME) and SOCIAL are highest priority.
        """
        c = self.cfg
        n = self.pop
        if n < 2:
        ...
        # Only organisms with SOCIAL and high relationship can merge
        has_social = self.module_active[:, M_SOCIAL]
        eligible = (has_social
                   & (self.relationship_score >= c.endo_relationship_threshold)
        ...
        # Each merger creates one new entity and removes both partners.
        nm = len(mergers_a)
        ma = np.array(mergers_a)
        mb = np.array(mergers_b)

        # Composite modules: union of both parents, capped at endo_max_modules.
        # When union exceeds cap, lowest-priority modules are dropped.
        # Priority hierarchy: energy sources (10) > social (9) > movement (8) > ...
        # This means MEDIATE (3) and VRESIST (4) get shed first from overloaded composites.
        ...
        # Remove both partners (keep newly appended composites).
        # Since _append_organisms already added nm composites at the end,
        # we build a keep-mask that's False for both partner sets in the
        # original population and True for the new composites.
        old_pop = self.pop - nm  # pop already increased by append
    ...
    def _decide_movement(self, readings):
        ...
        # Behavioral hijacking (Step 8): override movement for infected organisms.
        # Hijacked organisms seek high-density, low-viral areas to maximize virus spread.
        # Heavy hijack (>0.7 intensity) also adds erratic movement noise.
    ...
    def _reproduce(self):
        ...
        self._append_organisms({
            ...
            "merger_count": self.merger_count[pidx].copy(),           # inherit merger lineage from parent
        })
    ...
    def update(self):
        """Execute one timestep. Order of operations:
          1. Density recount → 2. Sense (22 channels) → 3. Hijack intensity
          4. Energy acquisition (with hijack suppression) → 5. Toxic damage + detox
          6. Module costs (with movement complexity penalty) → 7. Movement (with hijack)
          8. Age+1 → 9. Social field + interactions → 10. Mediator field
          11. Nutrient cycling → 12. Predation → 13. HGT (every 5)
          14. Viral dynamics (every 3) → 15. ENDOSYMBIOSIS (every 10) ← NEW
          16. Reproduce → 17. Kill & decompose → 18. Environment → 19. Stats
        """
        if self.pop == 0:
        ...
        # (Sessile crowding penalty removed in Step 9 — sessile producers compete
        # through canopy/roots, not escape. The sessile bonus already compensates.)
        ...
        # Endosymbiosis: rare merger events (every 10 steps) — Step 9 capstone.
        # Two compatible organisms in the same cell merge into a composite entity.
        mergers = 0
    ...
    def _record_stats(self, kills=0):
        ...
        if p > 0:
            ...
            self.stats_history.append({
                ...
                # Endosymbiosis stats (Step 9 — the capstone)
                "total_mergers": self.total_mergers,
                "max_merger_count": int(self.merger_count.max()) if p > 0 else 0,
                "composite_organisms": int((self.merger_count > 0).sum()),  # organisms with merger history
            })
        else:
            self.stats_history.append({
...
def run_simulation(cfg=None):
    ...
    print(f"The Shimmering Field — Phase 3 Step 9: Endosymbiosis (Capstone)")
