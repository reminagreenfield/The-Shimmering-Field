"""
The Shimmering Field — Phase 3 Step 6: MEDIATE + Nutrient Cycling
=================================================================
Introduces pollinator/disperser networks and active nutrient cycling.

New module:
  - MEDIATE: [pollination_drive, route_memory, network_coordination, reward_sensitivity]
             Organisms with MEDIATE facilitate reproduction for nearby organisms,
             forming pollinator networks. Network density/connectivity creates
             reproductive infrastructure that can later collapse (Phase 4 failure mode).

New system — Nutrient Cycling:
  Not a separate module but emergent from existing module interactions:
  - DETOX byproducts enrich local nutrients (detox_eff × conversion_rate)
  - CONSUME decomp processing releases nutrients (handling_efficiency)
  - FORAGE cooperative clusters boost local nutrient regeneration
  - Dead organisms deposit nutrients proportional to module complexity

Key interactions:
  - MEDIATE + reproduction: mediators reduce reproduction threshold for nearby organisms
  - MEDIATE + SOCIAL: mediators follow social signal gradients to find organisms
  - MEDIATE + movement: route_memory encodes productive patrol routes
  - Nutrient cycling creates spatial heterogeneity that drives ecological structure
  - Mediator network collapse = reproduction crisis for dependent populations

Built on Phase 3 Step 5 (VRESIST + SOCIAL).
"""
...
def run_simulation(cfg=None):
    ...
    print(f"The Shimmering Field — Phase 3 Step 6: MEDIATE + Nutrient Cycling")
