...
Endosymbiosis pathway:
  Prerequisites (accumulated over time):
    1. SOCIAL module on both organisms (relationship_score > threshold)
    2. Metabolic complementarity (different energy modules)
    3. Co-location in same cell
    4. Medium environmental stress (toxic in sweet spot)
    5. Both have sufficient energy

  Merger event:
    - Creates one new composite organism
    - Module inventory = union of both parents' modules
    - Weights = blended from both parents
    - Energy = sum (capped)
    - merger_count incremented (lineage tracking)
    - Both original organisms removed

Key interactions:
  - SOCIAL relationship_score is the gate — requires sustained compatible proximity
  - Metabolic complementarity ensures merger creates something novel
  - Medium stress window: too peaceful = no pressure to merge, too harsh = can't survive merging
  - Merger history enables readability of endosymbiotic lineages in art pipeline
  - VRESIST organisms less likely to merge (immune system treats partner as foreign)
  - Composite organisms have more modules → higher maintenance → must be in rich environment

Completes Phase 3: all module types, full two-layer genome, differentiated ecological roles.
...
class Config:
    ...
    endo_complementarity_min = 2       # min module difference count between partners
