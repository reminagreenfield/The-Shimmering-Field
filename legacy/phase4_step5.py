...
Fungal networks are NOT discrete organisms. They are a grid-level system
that emerges from decomposition and mediates nutrient/genome transport.

Growth mechanics:
  - Fungal density grows where decomposition is high (feeds on dead material)
  - Spreads via diffusion to neighboring cells (mycelial growth)
  - Expands rapidly during decomposition surges (mass death events)
  - Decays slowly in low-decomp areas (maintenance cost)

Transport functions:
  - Nutrient redistribution: moves nutrients from decomp-rich to decomp-poor zones
  - Genome fragment transport: HGT material travels along fungal paths
  - Non-uniform diffusion: fungal presence modifies toxic/nutrient diffusion rates
    (conduits in some directions, buffers in others)

Key interactions:
  - Mass death → decomp surge → fungal expansion → nutrient redistribution → recovery
  - Fungal networks make distant decomposition accessible to organisms
  - Toxic diffusion accelerated along fungal conduits (can propagate crises)
  - Genome fragments from collapsed populations carried to new zones via fungal transport
  - Creates the "what grows back carries history of what it replaced" dynamic
