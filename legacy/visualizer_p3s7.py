"""
The Shimmering Field — Phase 3 Step 7 Live Visualizer
======================================================
Runs the simulation and renders it in real-time using Pygame.

Usage:
    pip install pygame numpy scipy
    python visualizer_p3s7.py

Place this file in the same directory as phase3_step7.py.
...
# Precompute CONSUME weight offset once
...
def draw_organisms(surface, world, scale, color_mode="role", y_offset=0):
    ...
    if color_mode == "role":
        ...
        # Visualize lysogenic manipulation state:
        #   Green  = clean (no lysogenic material)
        #   Yellow = carrier (lysogenic but below manipulation threshold)
        #   Orange→Red = manipulated (above threshold, actively biasing reproduction)
        #   White ring = VRESIST organisms (immune to manipulation)
        thresh = world.cfg.repro_manip_threshold
        ls = world.lysogenic_strength
        has_vresist = world.module_active[:, M_VRESIST]

        # Base: green for clean organisms
        colors[:] = [40, 180, 60]
...
def draw_stats_panel(surface, world, x_offset, color_mode, steps_per_frame,
    ...
    lines.append(("Phase 3 Step 7: Repro Manipulation", (140, 130, 170)))
    lines.append(("", None))
    ...
    if s:
        lines.append((f"  Relationships: {s.get('avg_relationship', 0):.2f} avg", (130, 210, 255)))
    ...
    cm_names = {
        ...
        "manipulation": "Manipulation", "age": "Age",
    }
...
def main():
    pygame.init()
    pygame.display.set_caption("The Shimmering Field — Phase 3 Step 7")
    ...
    color_modes = ["role", "energy", "modules", "generation", "defense", "social", "manipulation", "age"]
    color_idx = 0
